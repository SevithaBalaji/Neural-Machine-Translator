...
def build_vocab(filename, normalize_digits=True):
    print("Building vocab for file : ", filename)
...
def token2id(data, mode):
    ...
    print("token to id for file : ", data, mode)
...
def train():
    ...
    with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:
        op = sess.run(tf.global_variables_initializer())
        #print(op)
        total_loss = 0
        ...
        while iteration < 12000:
            ...
            iteration += 1
            if iteration % 100 == 0:
                save_path = saver.save(sess, 'model/NMT', global_step=model.global_step)
...
def test():
    ...
    with open('tst2013.en', 'r', encoding='utf-8') as f:
        en_lines = f.readlines()

    with open('tst2013.vi', 'r', encoding='utf-8') as f:
        vi_lines = f.readlines()

    if (len(en_lines) != len(vi_lines)):
        print("ERROR Line difference")
        return
    ...
    with tf.Session() as sess:
        ...
        while i < len(en_lines) and i < len(vi_lines):
            ...
            i = i + 1
    print("Average BLUE score", np.mean(BLUE_score))
